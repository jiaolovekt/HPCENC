...
while getopts p:n:i:r:l:c:h OP ; do
	...
	;;
	c)
		[[ "$OPTARG" =~ 264|avc|AVC ]] && X264_EN=1
		[[ "$OPTARG" =~ 265|hevc|HEVC ]] && X265_EN=1
...
if [ "$X264_EN" = "1" ] && [ "$X264_AUD" = "AAC" ] ;then
	...
	if [ -f "$X264_AUD_TMP" ] && [ "$SkipEAudio" != "0" ] ; then
		...
		if [ "$DRYRUN" = 0 ] ; then
		"$FFMPEG_exec" -i "${SRCFILE}" -vn -c:a copy "$X264_AUD_TMP" -y
		else
		...
		#encode
		if [ "$DRYRUN" = 0 ] ; then
		"$FFMPEG_exec" -i "$AVSFILE" -vn -c:a aac -q "$AAC_Q" "$X264_AUD_TMP" -y
		else
...
if [ "$X265_EN" = "1" ] && [ "$X265_AUD" = "AAC" ] ;then
	...
	if [ -f "$X265_AUD_TMP" ] && [ "$SkipEAudio" != "0" ] ; then
		...
		logg "Audio bit rate $Audiorate lower than $AudDirectmuxthreshold, direct mux" info
		if [ "$DRYRUN" = 0 ] ; then
		"$FFMPEG_exec" -i "${SRCFILE}" -vn -c:a copy "$X265_AUD_TMP" -y
		else
		...
		#encode
		if [ "$DRYRUN" = 0 ] ; then
		"$FFMPEG_exec" -i "$AVSFILE" -vn -c:a aac -q "$AAC_Q" "$X265_AUD_TMP" -y
		else
...
if [ "$X264_EN" = "1" ] ; then
	if [ "$DRYRUN" = 0 ] ; then
	"$FFMPEG_exec" -i "$X264_TMP" -i "$X264_AUD_TMP" -c:v copy -c:a copy -metadata comment="$MCOMMENT" -map 0:v -map 1:a "${OUTDIR}/${OUTNAME}.${X264_MUX}" -y
	else
...
if [ "$X265_EN" = "1" ] ; then
	if [ "$DRYRUN" = 0 ] ; then
	"$FFMPEG_exec" -i "$X265_TMP" -i "$X265_AUD_TMP" -c:v copy -c:a copy -metadata comment="$MCOMMENT" -map 0:v -map 1:a "{$OUTDIR}/${OUTNAME}.${X265_MUX}" -y
	else
